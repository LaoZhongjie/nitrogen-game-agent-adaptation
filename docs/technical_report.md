...
> **版本：** v1.3（v1.2 + 实验章：微调前/短训对照与【编造】对比表）  
> **日期：** 2026年4月4日  
> **作者：** *[待填写]*  
...
9. [离线评估模块](#9-离线评估模块)
10. [端到端流水线](#10-端到端流水线)
11. [实验结果与分析（示意 + 对比）](#11-实验结果与分析示意--对比)
12. [技术实现细节](#12-技术实现细节)
...
> **说明（必读）**  
> - 与 **代码、配置一致** 的仅为：**评估协议、指标定义、数据规模量级**（如 `num_train_steps=5000`、分辨率 **160×288**、每段 **13** 帧、`GENERATE_MAX_SAMPLES=5`、池化约 **65** 帧等）。  
> - **凡标注【编造】的表格与数值均为虚构示例**，用于展示「如何读指标、如何与微调前/弱基线对比」；**非本仓库自动导出的实测结果**。复现实验需在你方环境实际运行 `predict` + 评估流水线后替换为真值。

### 11.1 数据子集概况

| 项目 | 值 | 备注 |
|------|-----|------|
| 设计上限 chunk 数 | ≤ 150 | 与 `main.py` 中 3 shards × 50 chunks/shard 一致 |
| 含有效 `video.mp4` 的 chunk（【编造】） | ~132 | 合理假设：部分 chunk 缺视频或元数据不完整 |
| Train / Val / Test（【编造】） | ~106 / ~13 / ~13 | 与 0.8/0.1/0.1、`split_granularity="chunk"` 量级一致 |
| 每 chunk 训练/推理帧数 | 13 | 与 `configs/finetune.example.json` 中 `num_frames` 及 `nearest_valid_frame_count` 一致 |

### 11.2 训练行为（【编造】示意曲线量级）

| 项目 | 值 | 与仓库关系 |
|------|-----|------------|
| 优化步数 | 5000 | 与 `num_train_steps` 一致 |
| 末 epoch 验证流匹配 MSE（【编造】） | ~0.108 | 与流匹配目标同量纲，随数据与实现会变 |
| `best_model` 对应 train loss（【编造】） | ~0.095 | 与代码「按 epoch 平均 train loss 选优」一致 |
| Wall-clock（【编造】） | ~14 h | 单卡 **A100 80GB**、NF4+VAE 编码的合理量级 |

### 11.3 对比实验设置（协议与仓库一致，场景【编造】）

**目的**：离线指标 **必须放在对照系里** 才有「好坏」；单独一个 **LPIPS=0.44** 无法判断是否足够好。

**固定条件（与实现一致）**：同一批 **val** 片段（**5** 个 chunk）、每段 **13** 帧、分辨率 **160×288**、**同一随机种子**、**同一 `GamepadActionEncoder` 文本条件**、**同一参考 `video.mp4` 对齐与 v2 评估代码路径**。仅 **检查点** 不同。

**对照组定义**：

| 代号 | 条件 | 含义 |
|------|------|------|
| **Base** | **微调前** | 加载 **`hunyuanvideo-community/HunyuanVideo`**，**不加载 LoRA**；提示与微调后完全相同。代表「通用视频先验 + NitroGen 动作文本」但 **未在 NitroGen 上适配**。 |
| **LoRA-800** | **短训消融** | 与正式训练 **同结构 LoRA**，仅 **【编造】800 steps**（约 **正式 5000 steps 的 16%**），其余超参相同。代表 **欠拟合 / 训练不足**。 |
| **LoRA-5k** | **本文方法** | **`num_train_steps=5000`**，与 `finetune.example.json` 一致，加载 **`best_model`** 权重。 |

**指标优劣约定**（读表前先看）：

- **越低越好**：`mean_lpips`、`mean_mae`、`mean_temporal_error_vs_reference`、`inception_feature_mean_l2`、`mean_fid`。  
- **越高越好**：`mean_psnr`、`mean_ssim`。  
- **`mean_temporal_consistency`**：仅反映生成序列 **是否平滑**；**不等于**与参考一致——需对照 **`mean_reference_temporal_consistency`**，避免把「糊成一片」当成好结果。

### 11.4 离线指标对比表（【编造】）

以下数值在同一评估脚本、同一 5×13 池化帧上 **手工设定的合理相对关系**：Base 最差，短训居中，5k 最优；幅度参考 **低分辨率动作条件视频生成** 常见现象（基底易语义漂移、细节对不齐；LoRA 后像素与感知误差下降）。

| 指标 | Base（微调前） | LoRA-800（短训） | LoRA-5k（本文） | 参考真值侧（对照） |
|------|----------------|------------------|-----------------|---------------------|
| `mean_lpips` ↓ | 0.61 | 0.52 | **0.44** | — |
| `mean_psnr` ↑ | 17.4 dB | 19.2 dB | **20.8 dB** | — |
| `mean_mae` ↓ | 14.8 | 11.6 | **9.2** | — |
| `mean_ssim` ↑ | 0.41 | 0.49 | **0.56** | — |
| `inception_feature_mean_l2` ↓ | 1.28 | 0.98 | **0.82** | — |
| `mean_fid` ↓ | 124 | 88 | **64** | — |
| `mean_temporal_error_vs_reference` ↓ | 11.4 | 8.9 | **6.5** | — |
| `mean_temporal_consistency` | 0.83 | 0.86 | **0.87** | `mean_reference_temporal_consistency` **0.89**（【编造】，同批参考序列） |

**相对 Base 的改善（【编造】，仅便于汇报）**：LoRA-5k 相对微调前约 **LPIPS −28%**、**PSNR +3.4 dB**、**MAE −38%**、**FID −48%**（均为粗略百分比，基于上表四舍五入）。

### 11.5 分析（基于上表【编造】数据的解释框架）

1. **为何必须与微调前比**：Base 在 **相同动作文本** 下仍与 NitroGen 参考差一截，说明 **通用先验 alone 不足以对齐「手柄→像素」**；改善主要来自 **在目标域片段上的流匹配 LoRA**，而非指标本身「看起来绝对值还行」。  
2. **短训 LoRA-800 的角色**：各指标 **介于 Base 与 LoRA-5k 之间**，支持「**训练步数不足则欠拟合**」的判断；若你方实测中短训与满训接近，需检查 **学习率、数据量、或是否早停过早**。  
3. **TC 与 `temporal_error_vs_reference` 分工**：三者 TC（Base < 短训 < 满训）略升，说明生成 **并未因拟合而剧烈抖动**；**`temporal_error_vs_reference` 降幅更大**，更直接反映 **帧间变化模式是否贴近参考**，比单独 TC 更有判别力。  
4. **FID 与 `inception_feature_mean_l2`**：二者在表中 **同向改善**，符合「分布更接近参考特征云」的预期；**小样本 FID 噪声大**，汇报时建议 **始终同时给出 L2 与 LPIPS**，避免单点解读。  
5. **上界与下界心理锚点**：**完美复现** 在像素上对应 **MAE→0、PSNR→∞**（实际不可能）；**Base** 提供 **下界锚点**；**LoRA-5k** 是当前配置下的 **主结果**。若未来加入 **更强基线**（如全参微调、更大分辨率），应 **另开一行** 扩展本表而非替换「微调前」对照。

### 11.6 工程上如何复现本表结构（非编造）

在 **同一 `generation_manifest.json` 结构** 下，对 **Base** 跑一遍生成（不加载 `best_model` 目录下的 LoRA），再对 **不同 checkpoint** 各跑一遍，使用 **同一 `build_evaluation_records` + v2 报告** 即可得到 **可替换上表的真实数字**；将结果按 **11.4** 制表即形成可发表的对比实验节。
...
> **文档说明**：第 11 章中 **标注【编造】的对比表与曲线数值** 为 **虚构示例**，用于说明 **与微调前/短训对照** 的写法与合理量级；**未标注【编造】** 的小节仍与配置/代码一致。其余章节与 `main.py`、`configs/finetune.example.json` 及 `src/` 下实现 **逐项对齐**（文档版本 **v1.3** 对应 **v2 评估报告** 与 **实验对比写法**）。若实现变更，请同步更新本章与文首版本号。
